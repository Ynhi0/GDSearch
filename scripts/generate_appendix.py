...
def get_environment_info() -> Dict:
    ...
    # AUDIT FIX: Use datetime.utcnow() for actual UTC time, or label as local time
    return {
    ...
    for csv_path in csv_files:
        ...
        try:
            ...
            if not eval_df.empty:
                # AUDIT FIX: Safely extract last non-NA element with fallback to test_acc if test_accuracy missing
                    ...
                    try:
                        if hasattr(col, 'dropna'):
                            try:
                                ...
                                import logging
                                logging.debug("generate_appendix: dropna failed for test_accuracy in %s: %s", csv_path, e, exc_info=True)
                                arr = np.asarray(col)
                    except Exception as e:
                        import logging
                        logging.debug("generate_appendix: error extracting test_accuracy in %s: %s", csv_path, e, exc_info=True)
                        arr = np.asarray(col)
                    config['final_test_accuracy'] = float(arr.ravel()[-1]) if arr.size > 0 and not pd.isna(arr.ravel()[-1]) else None
                except Exception as e:
                    import logging
                    logging.debug("generate_appendix: could not compute final_test_accuracy for %s: %s", csv_path, e, exc_info=True)
                ...
                try:
                    ...
                    try:
                        ...
                        import logging
                        logging.debug("generate_appendix: dropna branch failed for %s: %s", csv_path, e, exc_info=True)
                    config['final_test_loss'] = float(arr.ravel()[-1]) if arr.size > 0 and not pd.isna(arr.ravel()[-1]) else None
                except Exception as e:
                    import logging
                    logging.debug("generate_appendix: could not compute final_test_loss for %s: %s", csv_path, e, exc_info=True)
...
def generate_appendix_markdown(output_path: str = 'APPENDIX.md'):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
                            ...
                            if callable(drop):
                                try:
                                    ...
                                    try:
                                        ...
                                        import logging
                                        logging.debug("generate_appendix: filtering lrs failed for %s: %s", optimizer, e, exc_info=True)
                                ...
                                try:
                                    ...
                                    import logging
                                    logging.debug("generate_appendix: filtering lrs else-branch failed for %s: %s", optimizer, e, exc_info=True)
                                lrs = np.unique(arr) if arr.size > 0 else np.array([])
                            if len(lrs) > 0:
                            ...
                            if callable(drop):
                                try:
                                    ...
                                    try:
                                        ...
                                        import logging
                                        logging.debug("generate_appendix: filtering moms failed for %s: %s", optimizer, e, exc_info=True)
                                ...
                                try:
                                    ...
                                    import logging
                                    logging.debug("generate_appendix: filtering moms else-branch failed for %s: %s", optimizer, e, exc_info=True)
                                moms = np.unique(arr) if arr.size > 0 else np.array([])
                            if len(moms) > 0:
                            ...
                            if callable(drop):
                                try:
                                    ...
                                    try:
                                        ...
                                        import logging
                                        logging.debug("generate_appendix: filtering wds failed for %s: %s", optimizer, e, exc_info=True)
                                ...
                                try:
                                    ...
                                    import logging
                                    logging.debug("generate_appendix: filtering wds else-branch failed for %s: %s", optimizer, e, exc_info=True)
                                wds = np.unique(arr) if arr.size > 0 else np.array([])
                            if len(wds) > 0:
        ...
        f.write("**GDSearch Version:** 2.0.0 Production Ready\n")
