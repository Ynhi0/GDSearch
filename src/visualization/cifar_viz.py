...
def create_cifar10_visualizations(results_dir: Path, csv_files: List[Path]) -> Dict[str, Path]:
    ...
    if 'epoch' in combined_df.columns and 'train_loss' in combined_df.columns and 'optimizer' in combined_df.columns:
        ...
        from .plotting_utils import plot_protect
    ...
    if final_df is None or final_df.empty:
        ...
        try:
            ...
            logger.debug('Could not clean final results for plotting', exc_info=True)
            final_df = None
        ...
        if final_df is None or final_df.empty:
            ...
            try:
                ...
                logger.debug('Could not write final comparison meta file', exc_info=True)
