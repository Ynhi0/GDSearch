...
def theoretical_adam(iterations: np.ndarray, _L: float,
                    initial_loss: float, _alpha: float = 0.001,
                    _beta1: float = 0.9, _beta2: float = 0.999) -> np.ndarray:
    """
    ...
    Args:
        iterations: Array of iteration numbers
        _L: Lipschitz constant (unused in simplified bound)
        initial_loss: f(x_0) - f*
        _alpha, _beta1, _beta2: Adam hyperparameters (unused in simplified bound)
...
def fit_empirical_rate(iterations: np.ndarray, values: np.ndarray,
    ...
    try:
        ...
        logging.info("Warning: Curve fitting failed: %s", e)
        return 0.0, np.zeros_like(iterations), 0.0
...
def compare_theory_practice(training_csv: str, optimizer_name: str,
    ...
    **COMPLIANCE UPDATE**: Now properly uses PL-constant for non-convex theory curves.
    ...
    if 'iteration' not in df.columns or 'loss' not in df.columns:
        logging.info("Warning: CSV must have 'iteration' and 'loss' columns")
        return {}
    ...
    if pl_constant is not None and pl_constant > 1e-12:
        ...
        # GUARD: Check for invalid mu/L ratio
        if pl_constant >= L:
            logging.warning("Invalid PL parameters: pl_constant=%.4f >= L=%.4f. Skipping theoretical comparison.", pl_constant, L)
            return {}
        ...
        # GUARD: Check for invalid mu/L ratio
        if mu >= L:
            logging.warning("Invalid strongly-convex parameters: mu=%.4f >= L=%.4f. Skipping theoretical comparison.", mu, L)
            return {}
    ...
    # Additional guard: Check for NaN or negative values in theoretical bound
    if np.any(~np.isfinite(theoretical)) or np.any(theoretical < 0):
        logging.warning("Theoretical bound contains invalid values (NaN or negative). Skipping comparison.")
        return {}
    ...
    # AUDIT FIX: Add edge-case guards to prevent log-of-negative and divide-by-zero
    if mu is not None and empirical_rate > 0 and theoretical_rate is not None:
        ...
        # Guard against edge cases: theoretical_rate must be in (0, 1) for valid log
        if theoretical_rate <= 0 or theoretical_rate >= 1:
            logging.warning("Invalid theoretical_rate=%.6f (mu/L=%.4f). Skipping optimality gap computation.", theoretical_rate, mu/L)
            theoretical_rate_log = None
        else:
            theoretical_rate_log = -np.log(theoretical_rate)

        # Optimality gap: how far is empirical rate from theoretical rate?
        if theoretical_rate_log is not None and theoretical_rate_log > 1e-12:
        ...
        if theoretical_rate_log is not None:
            logging.info("  Theoretical convergence rate (ρ): %.6f", theoretical_rate)
            logging.info("  Theoretical rate (-log ρ): %.6f", theoretical_rate_log)
            logging.info("  Empirical rate (-log ρ_fit): %.6f", empirical_rate)

        if optimality_gap is not None and optimality_gap_pct is not None:
            logging.info("  ⚠️  OPTIMALITY GAP: %.6f (%.2f%%)", optimality_gap, optimality_gap_pct)

            if optimality_gap_pct > 10:
                logging.info("  WARNING: Large optimality gap (>10%%) indicates theory-practice mismatch")
            elif optimality_gap_pct < 5:
                logging.info("  ✓ Good agreement: optimality gap <5%%")
        else:
            logging.info("  Optimality gap computation skipped (edge-case detected)")
    ...
    ax2.semilogy(iterations, fitted_curve, 'g--', linewidth=2, label='Empirical Fit')
    ...
    logging.info("✓ Theory-practice comparison saved to %s", plot_path)
    ...
    logging.info("✓ Stats saved to %s", stats_csv)
...
def batch_compare_optimizers(results_dir: str, output_dir: str,
    ...
    for opt in optimizers:
        ...
        if len(csv_files) == 0:
            logging.info("Warning: No CSV found for %s", opt)
            continue
    ...
    if all_stats:
        ...
        logging.info("\n✓ Comparison table saved to %s", table_path)
