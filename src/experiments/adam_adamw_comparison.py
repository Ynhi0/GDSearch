...
def run_adam_vs_adamw_comparison(
    ...
    """
    logging.info("\n" + "="*80)
    logging.info("🔬 ADAM L2 vs. ADAMW COMPARISON")
    logging.info("="*80)
    logging.info("   Demonstrates why L2 regularization fails with adaptive optimizers")
    logging.info("="*80)
    ...
    if resume:
        ...
        if result_file.exists():
            try:
                ...
                logging.debug("Could not read adam_l2_vs_adamw_comparison.csv: %s", e, exc_info=True)
...
def run_lr_schedule_demonstration(
    ...
    if not history:
        ...
        print(f"  Failed: no history recorded")
    else:
        results.append({
            'schedule': 'Constant',
