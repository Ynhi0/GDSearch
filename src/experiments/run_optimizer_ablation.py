...
def run_optimizer_ablation(
    ...
    **SCIENTIFIC CAVEAT - Learning Rate Selection for 2D Functions:**
    ...
    for opt_name, optimizer in optimizers:
        ...
        # Use analytical constants for known test functions when available
        ...
        if diverged:
            ...
            # Only fall back to trajectory-based estimation as last resort.
            #
            # Trajectory-based estimation creates circular reasoning: using optimizer
            # data to estimate constants, then using those to "predict" same data.
            # This proves internal consistency but not theoretical compliance.
            #
            ...
            if analytical_L is not None and analytical_mu is not None:
                est_L = analytical_L
                est_mu = analytical_mu
                logging.debug(f"Using analytical constants for {opt_name}: L={est_L:.4f}, mu={est_mu:.4f}")
            else:
                # Fallback: estimate from trajectory (circular reasoning)
                try:
                    ...
                    logging.debug(f"Using trajectory-based estimates for {opt_name}: L={est_L:.4f}, mu={est_mu:.4f} [circular]")
                except Exception as e:
