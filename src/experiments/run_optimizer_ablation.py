    ...
    **SCIENTIFIC CAVEAT - Learning Rate Selection for 2D Functions:**
    ...
    for opt_name, optimizer in optimizers:
        ...
        if diverged:
            ...
            # SCIENTIFIC VALIDITY WARNING: Circular Reasoning in Theoretical Bounds
            # ======================================================================
            # The following code estimates L (smoothness) and mu (strong convexity)
            # from the trajectory generated by the optimizer itself. These estimates
            # are then used to compute "theoretical" convergence bounds.
            #
            # PROBLEM: This is circular reasoning (tautology). We are using the data
            # to estimate the constants, then using those constants to "predict" the
            # same data. This proves internal consistency, NOT theoretical compliance.
            #
            # CORRECT APPROACH: Estimate L and mu independently BEFORE running the
            # optimizer, either:
            #   1. Analytically (for known test functions like Rosenbrock)
            #   2. Via separate grid sampling of the loss landscape
            #   3. Using test_function.hessian() at multiple points
            #
            # The current visualization is useful for "overlay comparison" but should
            # NOT be cited as "validation of theoretical bounds" in research papers.
            # ======================================================================
