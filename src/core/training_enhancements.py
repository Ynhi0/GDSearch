...
if __name__ == '__main__':
    ...
    # Test disk guardian (path configurable via DISK_GUARDIAN_PATH env var)
    import tempfile, os
    guardian_dir = Path(os.environ.get('DISK_GUARDIAN_PATH', tempfile.gettempdir())) / 'test_ckpts'
    guardian = DiskSpaceGuardian(str(guardian_dir), min_free_gb=0.1)
    free_gb = guardian.get_free_space_gb()
    logging.info("Free disk space: %.2fGB", free_gb)
    logging.info("Can save 500MB checkpoint: %s", guardian.can_save_checkpoint(500))
